...
public class AccurateMouse {
    ...
    private static boolean action(Clickable clickable, boolean hover, String... clickActions) {
        ...
        if (clickable instanceof RSCharacter) {
            ...
            name = ((RSItem) clickable).getDefinition().getName();
        }
    ...
    public static Point getWalkingPoint(RSTile destination) {
        ...
        for (RSTile tile : new RSArea(destination, 1).getAllTiles()) {
            ...
            polygons.addAll(Arrays.stream(Objects.getAt(tile)).filter(object -> object.getDefinition().getActions().length > 0).map(RSObject::getModel).filter(java.util.Objects::nonNull).map(RSModel::getEnclosedArea).filter(java.util.Objects::nonNull).collect(Collectors.toList()));
            polygons.addAll(Arrays.stream(GroundItems.getAt(tile)).filter(object -> object.getDefinition().getActions().length > 0).map(RSGroundItem::getModel).filter(java.util.Objects::nonNull).map(RSModel::getEnclosedArea).filter(java.util.Objects::nonNull).collect(Collectors.toList()));
            polygons.addAll(Arrays.stream(NPCs.find(Filters.NPCs.tileEquals(tile))).filter(object -> object.getActions().length > 0).map(RSNPC::getModel).filter(java.util.Objects::nonNull).map(RSModel::getEnclosedArea).filter(java.util.Objects::nonNull).collect(Collectors.toList()));
        }
